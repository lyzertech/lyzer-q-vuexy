...
@section('title', 'Accuenergy Acuvim L-V4 - Modbus Dashboard')
...
@section('content')
            ...
            <div class="card h-100">
                <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                    <div class="card-title mb-0">
                        <h5 class="mb-1">Accuenergy <span class="badge bg-label-success">Acuvim L-V4</span></h5>
                        <p class="card-subtitle">Modbus RTU — COM4 / 19200 / Unit 1</p>
                    </div>
                    <div>
                                                    ...
                                                    <div class="row">
                                                        @if (is_array($group['data']))
                                                            ...
                                                            <div class="col-12 mb-3">
                                                                <form action="{{ url('modbus/acuviml-v4/write') }}"
            ...
            <div class="modal-content shadow-lg rounded-4 border-0">
                <div class="modal-header bg-success text-white rounded-top-4 py-3 px-4">
                    <h4 class="modal-title d-flex align-items-center gap-2" id="readingMeasurementModalLabel">
                        ...
                        <span class="text-white">Reading Measurement — Acuvim L-V4</span>
                    </h4>
                ...
                <div class="modal-footer bg-white rounded-bottom-4 px-4 py-3">
                    <button type="button" class="btn btn-outline-success px-4" id="btn-refresh-modal">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Update address & count to match actual Acuvim L-V4 register map
            // ---------------------------------------------------------------
            const MODBUS_CARDS = [
                { address: 4159, count: 7 }  // 4159–4165: Week, Year, Month, Day, Hour, Minute, Second
            ];

            const READ_URL = '/modbus/acuviml-v4/read/data/';

            // ---------------------------------------------------------------
            // Register definition for the Reading Measurement modal
            // Populate from the Acuvim L-V4 datasheet
            // ---------------------------------------------------------------
            const READING_LIST = {
                4159: { name: "Week",   category: "Date & Time", icon: "ti-calendar",  color: "info",    unit: "" },
                4160: { name: "Year",   category: "Date & Time", icon: "ti-calendar",  color: "primary", unit: "" },
                4161: { name: "Month",  category: "Date & Time", icon: "ti-calendar",  color: "primary", unit: "" },
                4162: { name: "Day",    category: "Date & Time", icon: "ti-calendar",  color: "success", unit: "" },
                4163: { name: "Hour",   category: "Date & Time", icon: "ti-clock",     color: "success", unit: "" },
                4164: { name: "Minute", category: "Date & Time", icon: "ti-clock",     color: "warning", unit: "" },
                4165: { name: "Second", category: "Date & Time", icon: "ti-clock",     color: "warning", unit: "" },
            };
            ...
            // ---------------------------------------------------------------
            // Group write — batches changed inputs
            // ---------------------------------------------------------------
                ...
                for (let c of changes) {
                    ...
                    try {
                        ...
                        if (ok) {
                            ...
                            if (displayEl) displayEl.textContent = c.value;
            ...
            async function populateModal() {
                ...
                if (!contentEl) return;
                contentEl.innerHTML = '<div class="text-center py-5"><div class="spinner-border text-success" role="status"><span class="visually-hidden">Loading...</span></div><p class="text-muted mt-3">Loading measurement data...</p></div>';
                ...
                if (Object.keys(READING_LIST).length === 0) {
                    contentEl.innerHTML = '<div class="text-center text-muted py-5"><i class="ti ti-info-circle fs-3 mb-2 d-block"></i>Register map not yet configured.<br>Populate <code>READING_LIST</code> in <code>read.blade.php</code> from the Acuvim L-V4 datasheet.</div>';
                    return;
                }
                ...
                for (let cat in categories) {
                    ...
                    for (let item of categories[cat]) {
                        const color = colorMap[item.color] || '#71dd37';
                        html += `
            ...
            if (modalEl) modalEl.addEventListener('show.bs.modal', populateModal);

            const refreshBtn = document.getElementById('btn-refresh-modal');
            if (refreshBtn) refreshBtn.addEventListener('click', populateModal);

            // ---------------------------------------------------------------
            // Sync Time — POST to server, write PC time to registers 4159–4165
            // ---------------------------------------------------------------
            async function syncTime() {
                ...
                try {
                    const resp = await fetch('/modbus/acuviml-v4/sync-time', {
                        method: 'POST',
                        body: fd,
                        credentials: 'same-origin',
                        headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
